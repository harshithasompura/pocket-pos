...
type BusinessFormProps = { initial?: Partial<BusinessValues>; submitLabel: string; onSubmit: (values: BusinessValues) => Promise<void> };

const defaults: BusinessValues = {
  name: "", address: "", phone: "", gstNumber: "", currency: "INR", receiptWidth: 58,
  receiptFooter: "Thank you for your business", taxEnabled: false, defaultTaxPercentage: 0, inventoryTrackingEnabled: true,
};

export const BusinessForm = ({ initial, submitLabel, onSubmit }: BusinessFormProps) => {
  const { control, handleSubmit, formState: { errors, isSubmitting } } = useForm<BusinessInput, unknown, BusinessValues>({
    defaultValues: { ...defaults, ...initial }, resolver: zodResolver(businessSchema),
  });
  return <View style={styles.form}>
    <Controller control={control} name="name" render={({ field }) => <Field label="Business name" value={field.value} onBlur={field.onBlur} onChangeText={field.onChange} error={errors.name?.message} placeholder="Som Stores" />} />
    <Controller control={control} name="address" render={({ field }) => <Field label="Address" value={field.value} onChangeText={field.onChange} multiline placeholder="Shop address" />} />
    ...
    <Controller control={control} name="defaultTaxPercentage" render={({ field }) => <Field label="Default tax %" value={String(field.value)} onChangeText={(value) => field.onChange(Number(value) || 0)} keyboardType="decimal-pad" error={errors.defaultTaxPercentage?.message} />} />
