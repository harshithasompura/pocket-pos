...
describe("stockAdjustmentSchema", () => {
  it("requires a non-zero integer quantity change", () => {
    expect(
      stockAdjustmentSchema.safeParse({ movementType: "stock_added", quantityChange: 3 }).success,
    ).toBe(true);
    expect(
      stockAdjustmentSchema.safeParse({ movementType: "stock_added", quantityChange: 0 }).success,
    ).toBe(false);
  });
});
