...
export const ProductForm = ({ product, onSubmit }: ProductFormProps) => {
  ...
  const [price, setPrice] = useState(product ? String(product.sellingPricePaise / 100) : "");
  ...
  const submit = async () => {
    const parsed = productSchema.safeParse({
      ...
      sku,
      category,
      sellingPricePaise: Math.round(Number(price) * 100),
    ...
    <View style={styles.form}>
      ...
      <View style={styles.toggle}>
        <View style={styles.flex}>
          ...
          <Text style={styles.hint}>Stock changes will always create an audit entry.</Text>
        </View>
        <Switch
          value={tracked}
      ...
      <Button label={product ? "Save product" : "Add product"} loading={saving} onPress={submit} />
    </View>
  );
};
