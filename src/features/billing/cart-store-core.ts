...
const initial = {
  lines: [] as CartLine[],
  discount: { type: "none" } as Discount,
...
export const createCartState =
  (createLineId: () => string) =>
  (
    set: (update: Partial<CartState> | ((state: CartState) => Partial<CartState>)) => void,
  ): CartState => ({
    ...initial,
    addProduct: (product) =>
      set((state) => {
          ...
          : {
              lines: [
                ...state.lines,
                {
                  ...
                  quantity: 1,
                  affectsInventory: product.trackInventory,
                },
              ],
            };
      }),
...
export const createCartStore = () => {
  let nextId = 0;
  return createStore<CartState>(createCartState(() => `custom:${++nextId}`));
};
