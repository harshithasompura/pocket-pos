  ...
  await db.withExclusiveTransactionAsync(async (txn) => {
    ...
    for (const [productId, quantity] of required) {
      ...
      if (!product || product.is_active !== 1)
        throw new Error(
          `${input.lines.find((line) => line.productId === productId)?.name ?? "Product"} is unavailable`,
        );
      if (product.stock_quantity < quantity)
        throw new Error(`${product.name} only has ${product.stock_quantity} in stock`);
      products.set(productId, product);
    }

    const sequenceRow = await txn.getFirstAsync<{ value: string }>(
      "SELECT value FROM app_settings WHERE key = ?",
      "next_bill_sequence",
    );
    const sequence = Math.max(1, Number(sequenceRow?.value ?? 1));
    billNumber = formatBillNumber(sequence);
    await txn.runAsync(
    ...
    await txn.runAsync(
      `INSERT INTO bills (id,business_id,bill_number,subtotal_paise,discount_paise,tax_paise,total_paise,total_units,payment_method,status,print_status,pdf_uri,void_reason,created_at,voided_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      billId,
      input.businessId,
      billNumber,
    ...
    for (const line of input.lines) {
      await txn.runAsync(
        `INSERT INTO bill_items (id,bill_id,product_id,product_name_snapshot,sku_snapshot,quantity,unit_price_paise,line_total_paise,affects_inventory,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)`,
        createId(),
        billId,
        line.productId,
        line.name,
