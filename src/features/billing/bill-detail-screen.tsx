...
export const BillDetailScreen = ({ id }: { id: string }) => {
  ...
  const print = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Printing failed",
        `${errorMessage(error)} Your bill remains saved and can be retried here.`,
      );
    } finally {
      setPrinting(false);
    }
  ...
  const requestVoid = () => {
    if (voidReason.trim().length < 3)
      return Alert.alert(
        "Reason required",
        "Enter at least 3 characters explaining why this bill is being voided.",
      );
  ...
  if (!bill)
    return (
      <Screen>
        <Text style={styles.meta}>Loading bill…</Text>
      </Screen>
    );
    ...
    <Screen scroll style={styles.screen}>
        ...
        {items.map((item) => (
          <View key={item.id} style={styles.item}>
            <View style={styles.flex}>
              ...
              <Text style={styles.meta}>
                {item.quantity} × {formatCurrency(item.unitPricePaise)}
                {item.sku ? ` · ${item.sku}` : ""}
              </Text>
            </View>
      ...
      <Card style={styles.summary}>
        <View style={styles.totalRow}>
          ...
          <Text>{formatCurrency(bill.subtotalPaise)}</Text>
        </View>
        <View style={styles.totalRow}>
          <Text style={styles.meta}>Discount</Text>
          <Text>− {formatCurrency(bill.discountPaise)}</Text>
        </View>
        <View style={styles.totalRow}>
          <Text style={styles.meta}>Tax</Text>
          <Text>{formatCurrency(bill.taxPaise)}</Text>
        </View>
        <View style={styles.totalRow}>
        ...
        {bill.status === "void" ? (
          <View style={styles.voidState}>
            <Text style={styles.voidTitle}>VOID</Text>
            <Text style={styles.voidReason}>{bill.voidReason}</Text>
            {bill.voidedAt && <Text style={styles.meta}>{formatDateTime(bill.voidedAt)}</Text>}
          </View>
        ) : (
          <>
            ...
            <Button
              label="Print receipt"
              loading={printing}
              disabled={sharing || voiding}
              onPress={print}
            />
            <Button
              label={bill.pdfUri ? "Share PDF" : "Create & share PDF"}
              variant="secondary"
              loading={sharing}
