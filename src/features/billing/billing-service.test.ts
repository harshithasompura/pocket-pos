...
const fakeDatabase = (stock = 3) => {
  ...
  const txn = {
    getFirstAsync: async (sql: string) =>
      sql.includes("app_settings") ? null : { ...productRow, stock_quantity: stock },
  ...
  const db = {
    withExclusiveTransactionAsync: async (callback: (value: typeof txn) => Promise<void>) =>
      callback(txn),
...
describe("completeBill", () => {
  ...
  it("saves snapshots and reduces tracked inventory in one transaction", async () => {
    ...
    expect(calls.some(({ sql, args }) => sql.includes("UPDATE products") && args[0] === 1)).toBe(
      true,
    );
