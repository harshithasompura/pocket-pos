...
export type BusinessRow = {
  ...
  inventory_tracking_enabled: number;
  created_at: string;
  updated_at: string;
};
...
export const mapBusiness = (row: BusinessRow): Business => ({
  ...
  defaultTaxPercentage: row.default_tax_percentage,
  inventoryTrackingEnabled: row.inventory_tracking_enabled === 1,
  createdAt: row.created_at,
  updatedAt: row.updated_at,
});
...
export const mapBill = (row: BillRow): Bill => ({
  id: row.id,
  businessId: row.business_id,
...
export const mapBillItem = (row: BillItemRow): BillItem => ({
  ...
  affectsInventory: row.affects_inventory === 1,
  createdAt: row.created_at,
});
