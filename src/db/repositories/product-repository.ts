...
export const createProductRepository = (db: SQLiteDatabase) => ({
  async list(): Promise<Product[]> {
    return (await db.getAllAsync<ProductRow>("SELECT * FROM products ORDER BY is_active DESC, name COLLATE NOCASE")).map(mapProduct);
  },
  ...
  async create(businessId: string, values: ProductValues): Promise<Product> {
    ...
    await db.withExclusiveTransactionAsync(async (txn) => {
      await txn.runAsync(
        ...
        id, businessId, values.name, values.sku || null, values.category || null, values.sellingPricePaise,
        values.stockQuantity, values.lowStockThreshold, values.trackInventory ? 1 : 0, values.isActive ? 1 : 0, now, now,
      );
      if (values.trackInventory && values.stockQuantity !== 0) {
        await txn.runAsync(
          ...
          createId(), id, "opening_stock", 0, values.stockQuantity, values.stockQuantity, "Opening stock", now,
        );
      }
    });
  ...
  async update(id: string, values: ProductValues): Promise<Product> {
    await db.runAsync(
      ...
      values.name, values.sku || null, values.category || null, values.sellingPricePaise, values.lowStockThreshold,
      values.trackInventory ? 1 : 0, values.isActive ? 1 : 0, nowIso(), id,
    );
    return (await this.get(id))!;
  },
  async setActive(id: string, isActive: boolean) {
    await db.runAsync("UPDATE products SET is_active=?,updated_at=? WHERE id=?", isActive ? 1 : 0, nowIso(), id);
  },
  async adjustStock(id: string, values: StockAdjustmentValues): Promise<Product> {
    await db.withExclusiveTransactionAsync(async (txn) => {
      ...
      const now = nowIso();
      await txn.runAsync("UPDATE products SET stock_quantity=?,updated_at=? WHERE id=?", after, now, id);
      await txn.runAsync(
        `INSERT INTO inventory_movements (id,product_id,movement_type,quantity_before,quantity_change,quantity_after,note,created_at) VALUES (?,?,?,?,?,?,?,?)`,
        createId(), id, values.movementType, row.stock_quantity, values.quantityChange, after, values.note || null, now,
      );
    });
    return (await this.get(id))!;
  },
