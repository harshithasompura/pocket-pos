...
export const seedDemoData = async (db: SQLiteDatabase) => {
  const businesses = createBusinessRepository(db);
  let business = await businesses.get();
  if (!business)
    business = await businesses.save(
      businessSchema.parse({ name: "PocketPOS Demo", currency: "INR", receiptWidth: 58 }),
    );
  const products = createProductRepository(db);
  if ((await products.list()).length > 0) return { created: 0 };
  const samples = [
    {
      name: "Masala Tea",
      sku: "TEA-001",
